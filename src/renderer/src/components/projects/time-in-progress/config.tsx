...
const project = 'time-in-progress'
...
export function Config() {
  ...
  const { data, isPending } = useQuery({
    queryKey: ['time-in-progress-config'],
  ...
  const { mutate: updateConfigMutation } = useMutation({
    ...
    onSuccess: (success) => {
      if (success) {
        ...
        // TODO; Clear the insta insights cache
      }
    }
  })
...
// TODO; Get thhe services configs that run time in progress.
