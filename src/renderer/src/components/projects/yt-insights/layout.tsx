...
import { AccountsDataWithPic, ApiYTInsightsAccount, Range } from '@shared/types'
...
type Props = {
  ...
  accountsRaw?: ApiYTInsightsAccount[]
  isFetching: boolean
  isPending: boolean
  interval: number
  refetch: () => void
...
export function YTInsightsLayout({
  selectedAccount,
  accountsRaw,
  isFetching,
  isPending,
      ...
      <div className="flex w-full justify-between items-center">
        <div className="flex items-center gap-2">
          <Label className="text-lg text-bold text-foreground">
            YouTube Insights {selectedAccount ? ` - ${selectedAccount}` : ''}
          </Label>
          <InsightsTools handleOpenEditMenu={() => console.log('todo')} />
        </div>
...
function InsightsTools({ handleOpenEditMenu }: { handleOpenEditMenu: () => void }) {
