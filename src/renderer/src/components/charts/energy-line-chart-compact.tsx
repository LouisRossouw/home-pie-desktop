...
import { Area, YAxis, XAxis, Tooltip, ComposedChart, ResponsiveContainer } from 'recharts'

export function EnerygyRemainingKwhChart({
  data
}: {
...
export function EnergyBurnKwhChart({
  data
}: {
...
const EnergyRemaininigTooltip = ({ active, payload, label }: any): JSX.Element | null => {
...
const BurnRateTooltip = ({ active, payload, label }: any): JSX.Element | null => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-background/90 backdrop-blur-sm border rounded-lg p-2 text-xs shadow-lg">
        <p className="text-muted-foreground mb-1">
        ...
        <div className="space-y-1">
          ...
          <p className="text-[10px] text-muted-foreground">
            Rate: {(payload.find((p) => p.dataKey === 'kwhPerHour')?.value as number)?.toFixed(3)}{' '}
            kWh/h
          </p>
        </div>
      </div>
...
export function EnergyTimeRemainingChart({
  data
}: {
  ...
  return (
    <div className="flex w-full h-24 justify-center items-center">
      <ResponsiveContainer width="100%" height="100%">
        <ComposedChart data={data ?? []} margin={{ top: 5, right: 5, bottom: 5, left: 5 }}>
          <defs>
            <linearGradient id="gradientTimeRemaining" x1="0" y1="0" x2="0" y2="1">
          ...
          {data && (
            <Tooltip
              content={<EnergyTimeRemainingTooltip />}
          ...
          <Area
            ...
            strokeWidth={2}
            fillOpacity={1}
            fill="url(#gradientTimeRemaining)"
...
const EnergyTimeRemainingTooltip = ({ active, payload, label }: any): JSX.Element | null => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-background/90 backdrop-blur-sm border rounded-lg p-2 text-xs shadow-lg">
        <p className="text-muted-foreground mb-1">
        ...
        <p className="font-bold flex items-center gap-1">
          ...
          {(payload[0].value as number)?.toFixed(1)} hours
        </p>
      </div>
    )
  }
