...
export default function MyFinancesOverviewRoute() {
  ...
  const addDebt = () => {
    ...
    setFormData(prev => {
      ...
      return {
        ...prev,
        debts: [...(prev.debts || []), { id, label: 'New Debt', totalAmount: 0, monthlyAllocation: 0, interestRate: 0, startMonth: selectedMonth, startYear: selectedYear }]
      }
    })
  }
  ...
  const totals = useMemo(() => {
    if (!formData) return { tax: 0, netIncome: 0, totalExpenses: 0, monthlySavings: 0, totalAllocations: 0, assetAllocations: 0, goalAllocations: 0, debtAllocations: 0, totalDebt: 0, grossIncome: 0 }
    ...
    return { tax, netIncome, totalExpenses, monthlySavings, totalAllocations, assetAllocations, goalAllocations, debtAllocations, totalDebt, grossIncome }
  ...
  const chartData = useMemo(() => {
    ...
    sortedRecords.forEach(r => {
      ...
      const totalDebt = (r.data.debts || []).reduce((sum, d) => {
    ...
    for (let i = 1; i <= months; i++) {
      ...
      const totalNetWorth = totalAssetValue + totalGoalValue + currentSavingsPool - totalDebtRemaining
              ...
              <CollapsibleTrigger asChild>
                <CardHeader className="flex flex-row items-center justify-between cursor-pointer hover:bg-primary/5 transition-colors">
                  <div>
                    <CardTitle className="flex items-center gap-2">
                      Debts & Liabilities
                      <ChevronDown className={`size-4 transition-transform ${isDebtsOpen ? 'rotate-180' : ''}`} />
                    </CardTitle>
                    <CardDescription>Track debt payoff with interest calculations</CardDescription>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="text-right hidden sm:block">
                      <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-semibold">Total Debt</p>
                      <p className="text-sm font-bold text-red-500">R {totals.totalDebt.toLocaleString()}</p>
                    </div>
                    <Button size="sm" variant="outline" onClick={(e) => { e.stopPropagation(); addDebt(); }} className="h-8 gap-1 border-primary/20 hover:bg-primary/5">
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 gap-1">
                    <div className="grid grid-cols-12 items-center gap-2 px-2 mb-1">
                      <Label className="col-span-2 text-[10px] font-black uppercase tracking-widest opacity-50">Label</Label>
                      <Label className="col-span-2 text-[10px] font-black uppercase tracking-widest opacity-50 text-center">Total</Label>
                      <Label className="col-span-2 text-[10px] font-black uppercase tracking-widest opacity-50 text-center">Remaining</Label>
                      <Label className="col-span-1 text-[10px] font-black uppercase tracking-widest opacity-50 text-center">Rate%</Label>
                      <Label className="col-span-2 text-[10px] font-black uppercase tracking-widest opacity-50 text-center text-blue-400">Monthly</Label>
                      <Label className="col-span-2 text-[10px] font-black uppercase tracking-widest opacity-50 text-right text-green-400">Paid</Label>
                      <div className="col-span-1" />
                    </div>

                    {(formData.debts || []).map((debt) => {
                      const { remainingAmount, totalPaid, monthsElapsed } = calculateDebtProgress(debt, selectedMonth, selectedYear)
                      const monthsLeft = debt.monthlyAllocation > 0 && remainingAmount > 0
                        ? Math.ceil(remainingAmount / debt.monthlyAllocation)
                        : Infinity
                      const percentPaid = debt.totalAmount > 0 ? ((totalPaid / debt.totalAmount) * 100) : 0

                      return (
                        <div key={debt.id} className="space-y-2 group p-2 rounded-lg hover:bg-primary/5 transition-colors border border-transparent hover:border-primary/10">
                          <div className="grid grid-cols-12 items-center gap-2">
                            <div className="col-span-2">
                              <Input
                                value={debt.label}
                            ...
                            <div className="col-span-2">
                              <Input
                                ...
                                onChange={(e) => handleDebtChange(debt.id, 'monthlyAllocation', e.target.value)}
                          ...
                          <div className="px-2 flex items-center justify-between text-[10px]">
                            ...
                            <span className="text-muted-foreground whitespace-nowrap">
                              {remainingAmount <= 0 ? 'Paid Off!' : monthsLeft === Infinity ? 'No allocation' : `${monthsLeft} months left`}
                            </span>
                          </div>
                        </div>
                      )
                  ...
                  <div className="mt-2 p-2.5 rounded-lg bg-red-500/5 border border-red-500/10 text-[11px] space-y-1">
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-red-400">R {totals.totalDebt.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Monthly Debt Payments:</span>
                      <span className="font-bold text-blue-400">R {totals.debtAllocations.toLocaleString()}</span>
                    </div>
                  </div>
                </CardContent>
              </CollapsibleContent>
